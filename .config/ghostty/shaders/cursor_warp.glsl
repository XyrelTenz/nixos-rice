...
// --- CONFIGURATION ---
vec4 TRAIL_COLOR; // Set dynamically inside mainImage
...
// --- FADED WARP CONFIGURATION ---
const float FADE_ENABLED = 1.0; // 1.0 to enable fade gradient along the trail
const float FADE_EXPONENT = 3.0; // exponent for fade gradient along the trail

// --- CIRCULAR RIPPLE CONFIGURATION ---
const float RIPPLE_ENABLED = 1.0; // 1.0 to enable circular ripple effect
const float RIPPLE_DURATION = 0.15; // ripple animation duration in seconds
const float RIPPLE_MAX_RADIUS = 0.022; // subtle max radius in normalized coords
const float RIPPLE_RING_THICKNESS = 0.006; // delicate ring thickness
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    if (lineLength > minDist && baseProgress < DURATION - 0.001) {
        ...
        // Faded gradient calculation along movement vector
    ...
    // --- 2. PERFECT CIRCULAR RIPPLE EFFECT ---
    if (RIPPLE_ENABLED > 0.5) {
        float rippleProgress = baseProgress / RIPPLE_DURATION;
        if (rippleProgress >= 0.0 && rippleProgress <= 1.0) {
            float easedProgress = ease(rippleProgress);
            float rippleRadius = easedProgress * RIPPLE_MAX_RADIUS;
            float fade = 1.0 - easeOutPulse(rippleProgress);

            // Calculate exact radial distance to cursor center for a true circle
            float dist = distance(vu, centerCC);
            float sdfRing = abs(dist - rippleRadius) - RIPPLE_RING_THICKNESS * 0.5;
            float antiAliasSize = antialising(sdfRing, BLUR);

            vec4 rippleColor = TRAIL_COLOR;
            newColor = mix(newColor, vec4(rippleColor.rgb, newColor.a), antiAliasSize * fade * rippleColor.a * 0.8);
        }
    }
