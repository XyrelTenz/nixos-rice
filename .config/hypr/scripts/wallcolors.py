...
def main():
    ...
    # --- tmux dynamic theme ---
    # Build a sourceable tmux config from the same matugen palette so the
    # statusline follows the wallpaper in real-time.
    acc  = pill["primary"]          # accent colour
    dim  = b["base03"]              # muted / inactive window text
    bg   = b["base00"]              # terminal background (used for text on accent)
    fg   = b["base07"]              # main foreground text
    bg2  = b["base02"]              # surface for message bar
    border = b["base01"]            # pane border

    # Nerd Font powerline rounded caps — gives the NvChad pill look.
    # U+E0B6 = left half-circle (open left)  
    # U+E0B4 = right half-circle (open right)
    L = "\ue0b6"   # 
    R = "\ue0b4"   # 

    tmux_lines = [
        "# Auto-generated by wallcolors.py — do not edit by hand.",
        "set -g status-position top",
        f'set -g status-style "bg=default,fg={fg}"',

        # Left pill:  session-name 
        f'set -g status-left "#[fg={acc},bg=default]{L}#[fg={bg},bg={acc},bold] #S #[fg={acc},bg=default]{R} "',
        "set -g status-left-length 30",

        # Active window pill:  index:name 
        f'set -g window-status-current-format "#[fg={acc},bg=default]{L}#[fg={bg},bg={acc},bold] #I:#W #[fg={acc},bg=default]{R}"',
        "set -g window-status-current-style \"\"",

        # Inactive windows — plain text, no pill
        f'set -g window-status-format " #I:#W "',
        f'set -g window-status-style "fg={dim}"',

        # Right pill:  time   date 
        f'set -g status-right "#[fg={acc},bg=default]{L}#[fg={bg},bg={acc},bold] %H:%M #[fg={acc},bg=default]{R}#[fg={dim},bg=default] %d-%b-%y "',
        "set -g status-right-length 60",

        f'set -g pane-border-style "fg={border}"',
        f'set -g pane-active-border-style "fg={acc}"',
        f'set -g message-style "bg={bg2},fg={fg}"',
        f'set -g message-command-style "bg={bg2},fg={fg}"',
    ]
    tmux_conf = CACHE / "tmux-colors.conf"
    tmux_conf.write_text("\n".join(tmux_lines) + "\n")
    # Reload every live tmux session so the change is instant.
    subprocess.run(
        ["tmux", "source-file", str(tmux_conf)],
        capture_output=True,
    )  # silently ignored if tmux is not running
